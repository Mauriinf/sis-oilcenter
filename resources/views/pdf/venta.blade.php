...
<body>
  <div class="header">
    ...
    <div class="container">
      <p style="font-size : 20px;"><b><u>REPORTE DE VENTAS</u></b></p>
      <p>OILCENTER</p>
    </div><br>
  ...
  <table class="table table-striped table-bordered table-sm" cellspacing="0" id="TablaVenta">
    ...
    <tbody>
      ...
      <tr>
        <td>{{$loop->index+1}}</td>
        <td>{{$vt->articulo->nombre}}</td>
        <td>{{$vt->venta->cliente->nombres.' '.$vt->venta->cliente->paterno
          .' '.$vt->venta->cliente->materno}}</td>
        <td>{{$vt->venta->vendedorV->nombres.' '.$vt->venta->vendedorV->paterno
          .' '.$vt->venta->vendedorV->materno}}</td>
        <td>{{$vt->cantidad}}</td>
        <td>{{number_format($vt->precio_venta,2)}}</td>
        <td>{{number_format($vt->venta->monto_cancelado,2)}}</td>
        <td>{{number_format($vt->venta->monto_deuda,2)}}</td>
        <td>{{ $vt->venta->precio_venta }}</td>
      </tr>
      @endforeach
    </tbody>
  </table>
